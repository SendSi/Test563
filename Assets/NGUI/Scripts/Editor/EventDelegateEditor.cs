...
static public class EventDelegateEditor
{
    ...
    static public bool Field(Object undoObject, EventDelegate del, bool removeButton, bool minimalistic)
    {
        ...
        if (removeButton && (del.target != null || del.isValid))
        {
            ...
            {
                var tField = typeof(EventDelegate).GetField("mCachedCallback", BindingFlags.Instance | BindingFlags.NonPublic);
                if (tField != null)
                {
                    var tCall = tField.GetValue(del) as EventDelegate.Callback;
                    if (tCall != null && tCall.Target != null && tCall.Method != null)
                    {
                        var tScriptName = tCall.Target.GetType().Name;
                        var tMethodName = tCall.Method.Name;

                        var tScriptPath = AssetDatabase.FindAssets("t:Script " + tScriptName, new string[] { "Assets" });
                        ...
                        if (tLineIndex > 1)
                            AssetDatabase.OpenAsset(tObj, tLineIndex / 2 + 1);
                    }
                }
            }
